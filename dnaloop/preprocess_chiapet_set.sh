...
LOG_FILE="preprocess_chiapet_set.log"

mkdir -p $CHIAPET_SET_DIR/set
echo "`date`: Run starting in $CHIAPET_SET_DIR" | tee $CHIAPET_SET_DIR/$LOG_FILE

rm -f $CHIAPET_SET_DIR/set/reads.bed
for SAMPLE_DIR in $SAMPLE_DIRS
do
    echo "`date`: Adding reads from $SAMPLE_DIR to reads.bed" | tee -a $CHIAPET_SET_DIR/$LOG_FILE
    cat $SAMPLE_DIR/left.dedup.bed $SAMPLE_DIR/right.dedup.bed >> $CHIAPET_SET_DIR/set/reads.bed
done

echo "`date`: Calling peaks using reads.bed to define interaction anchor locations" | tee -a $CHIAPET_SET_DIR/$LOG_FILE
macs2 callpeak -t $CHIAPET_SET_DIR/set/reads.bed -f BED -n anchor --nomodel -p 0.01 --outdir $CHIAPET_SET_DIR/set
echo "`date`: Found `cat $CHIAPET_SET_DIR/set/anchor_peaks.narrowPeak | wc -l` peaks" | tee -a $CHIAPET_SET_DIR/$LOG_FILE
bedtools merge -d 1500 -i $CHIAPET_SET_DIR/set/anchor_peaks.narrowPeak > $CHIAPET_SET_DIR/set/anchor_peaks.merged.bed
echo "`date`: Merged peaks within 1500bp resulting in `cat $CHIAPET_SET_DIR/set/anchor_peaks.merged.bed | wc -l` anchors" | tee -a $CHIAPET_SET_DIR/$LOG_FILE


for SAMPLE_DIR in $SAMPLE_DIRS
do
    echo "`date`: Processing $SAMPLE_DIR" | tee -a $CHIAPET_SET_DIR/$LOG_FILE
    echo "`date`:   Converting read pairs to anchor pairs by intersecting with peaks" | tee -a $CHIAPET_SET_DIR/$LOG_FILE
    SAMPLE_ANCHOR_DIR=$CHIAPET_SET_DIR/set/`basename $SAMPLE_DIR`
    mkdir -p $SAMPLE_ANCHOR_DIR
    bedtools intersect -loj -a $SAMPLE_DIR/left.dedup.bed -b $CHIAPET_SET_DIR/set/anchor_peaks.merged.bed | awk '{print $5,$6,$7,$4}' OFS='\t' > $SAMPLE_ANCHOR_DIR/anchor1.bed
    bedtools intersect -loj -a $SAMPLE_DIR/right.dedup.bed -b $CHIAPET_SET_DIR/set/anchor_peaks.merged.bed | awk '{print $5,$6,$7,$4}' OFS='\t' > $SAMPLE_ANCHOR_DIR/anchor2.bed
    ...
    # Create BEDPE
    paste $SAMPLE_ANCHOR_DIR/anchor1.bed $SAMPLE_ANCHOR_DIR/anchor2.bed | cut -f 1-3,5-8 > $SAMPLE_ANCHOR_DIR/loops.tmp
    # Keep only interactions where both reads map to anchors
    awk '{if ($1 != "." && $4 != ".") print}' $SAMPLE_ANCHOR_DIR/loops.tmp > $SAMPLE_ANCHOR_DIR/loops.bedpe
    echo "`date`:   Wrote `cat $SAMPLE_ANCHOR_DIR/loops.bedpe | wc -l` loops where both reads map to anchors ($SAMPLE_ANCHOR_DIR/loops.bedpe)" | tee -a $CHIAPET_SET_DIR/$LOG_FILE    
    cut -f1-6 $SAMPLE_ANCHOR_DIR/loops.bedpe | sort | uniq -c | awk '{print $2,$3,$4,$5,$6,$7,".",$1}' > $SAMPLE_ANCHOR_DIR/loop_counts.bedpe
    echo "`date`:   Wrote anchor interaction counts to $SAMPLE_ANCHOR_DIR/loop_counts.bedpe" | tee -a $CHIAPET_SET_DIR/$LOG_FILE
